...
function CloudPairingSection() {
  ...
  async function startPairing() {
    ...
    try {
      ...
      // Status poll picks up the new pairingState within ~1s.
      setCode("")
    } catch (err) {
  ...
  return (
    <div className="glass-card overflow-hidden">
      ...
      <div className="p-3 space-y-2">
        ...
        {!paired && !inFlight && (
          <>
            <p className="text-xs text-slate-500">
              Enter the 6-digit code from <span className="font-mono">sentryusb.com</span> →
              Settings → Devices → Pair a Pi.
            </p>
            <div className="flex items-center gap-2">
              <input
                type="text"
                ...
                placeholder="123456"
                className="font-mono w-32 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-center text-base tracking-[0.4em] text-slate-200 placeholder:text-slate-600 focus:border-sky-500/40 focus:outline-none"
              />
              <button
                onClick={startPairing}
                disabled={submitting || code.length !== 6}
                className="rounded-lg bg-sky-500 px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-sky-600 disabled:opacity-50"
              >
                {submitting ? (
        ...
        {/* In flight: show progress + cancel. */}
        {!paired && inFlight && (
        ...
        {/* Pairing failed: show error + retry. */}
        ...
        {/* Paired: show status + counters + unpair. */}
        {paired && status && (
          <>
            <div className="grid grid-cols-2 gap-2 text-[11px]">
              <div>
                <p className="text-slate-500">Pi ID</p>
                <p className="font-mono text-slate-300 truncate" title={status.piId ?? ""}>
                  {status.piId}
                </p>
              </div>
              <div>
                <p className="text-slate-500">Account</p>
                <p className="font-mono text-slate-300 truncate" title={status.userId ?? ""}>
                  {status.userId}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-slate-300">
                  {status.totalUploadedRouteCount}{" "}
            ...
            {confirmUnpair ? (
              <div className="flex items-center justify-between gap-2 rounded-lg border border-red-500/30 bg-red-500/10 px-2 py-1.5">
                <p className="text-[11px] text-red-300">
                  Unpair this Pi? Local credentials are wiped.
                </p>
                <div className="flex gap-1">
                  <button
                    onClick={unpair}
              ...
              <button
                ...
                className="text-[11px] text-slate-500 underline-offset-2 hover:text-red-400 hover:underline"
              >
                Unpair this Pi
              </button>
            )}
