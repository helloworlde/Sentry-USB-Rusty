...
pub struct DriveSummary {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tessie_autopilot_percent: Option<f64>,
    /// v17 Safety Score totals for this drive (see `safety.rs`). All
    /// `#[serde(default)]` so pre-v17 `drive_list_cache` JSON still
...
pub struct RouteAggregates {
    ...
    /// v17 Safety Score scalars (see `safety.rs`). `None` = row not yet
