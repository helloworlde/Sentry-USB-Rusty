...
///
/// v16 -> v17: add per-clip Safety Score scalars to `routes`
/// (V17_ROUTE_SAFETY_COLUMNS, see `safety.rs`): hard-braking /
...
///
/// v17 -> v18: add conditional-ratio denominator columns to `routes`
/// (V18_ROUTE_SAFETY_DENOM_COLUMNS): manual time decelerating above
...
/// existing rows by the same formula-version backfill as v17.
///
/// v18 -> v19: add `accel_x_blob` / `accel_y_blob` to `routes` — the SEI
...
/// blobs (pre-v19 extractions, firmware without the fields) fall back
...
/// v18 conditional-ratio denominators on `routes` (see `safety.rs`).
pub const V18_ROUTE_SAFETY_DENOM_COLUMNS: &[(&str, &str)] = &[
    ("safety_brake_any_ms", "INTEGER"),
    ("safety_turn_any_ms", "INTEGER"),
];

/// v19 IMU accel channels on `routes` (see `CURRENT_SCHEMA_VERSION`).
pub const V19_ROUTE_IMU_COLUMNS: &[(&str, &str)] = &[
    ("accel_x_blob", "BLOB"),
    ("accel_y_blob", "BLOB"),
];
...
pub fn migrate(conn: &Connection) -> Result<()> {
    ...
    for (name, typ) in V2_ROUTE_AGGREGATE_COLUMNS
        ...
        .chain(V17_ROUTE_SAFETY_COLUMNS.iter())
        .chain(V18_ROUTE_SAFETY_DENOM_COLUMNS.iter())
        .chain(V19_ROUTE_IMU_COLUMNS.iter())
    {
...
mod tests {
    ...
    #[test]
    fn migrate_from_v16_adds_v17_safety_columns() {
        // Stand up a v16 DB (everything but the v17 safety columns) and
        // confirm migrate adds them.
        let conn = open();
        for stmt in V1_SCHEMA {
            conn.execute(stmt, []).unwrap();
        ...
        let cols = list_route_columns(&conn).unwrap();
        for (name, _) in V17_ROUTE_SAFETY_COLUMNS {
            assert!(cols.contains(*name), "routes.{} missing after v17", name);
        }
        assert_eq!(
            meta_get(&conn, "schema_version").unwrap().as_deref(),
            Some("19")
    ...
    #[test]
    fn migrate_from_v17_adds_v18_denom_columns() {
        // Stand up a v17 DB (everything but the v18 denominator columns)
        ...
        let cols = list_route_columns(&conn).unwrap();
        for (name, _) in V18_ROUTE_SAFETY_DENOM_COLUMNS {
    ...
    #[test]
    fn migrate_from_v18_adds_v19_imu_columns() {
        // Stand up a v18 DB (everything but the IMU blobs) and confirm
        // migrate adds them.
        let conn = open();
        for stmt in V1_SCHEMA {
            conn.execute(stmt, []).unwrap();
        ...
        let cols = list_route_columns(&conn).unwrap();
        for (name, _) in V19_ROUTE_IMU_COLUMNS {
