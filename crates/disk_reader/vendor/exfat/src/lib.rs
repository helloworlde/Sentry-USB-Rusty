    ...
    pub fn open(mut image: I) -> Result<Self, OpenError> {
        ...
        loop {
            ...
            if ty.is_end() {
                ...
                // Unused (deleted) entry: skip, per the exFAT spec.
                continue;
            ...
            match (ty.type_importance(), ty.type_code()) {
                ...
                (importance, _) if importance != EntryType::CRITICAL => {
                    // Benign primary entry (e.g. Volume GUID): the spec
                    // allows ignoring it. Skip its secondaries too
                    // (SecondaryCount is byte 1).
                    let secondaries = entry.data()[1];
