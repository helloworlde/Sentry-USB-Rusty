...
impl<I: Read + Seek> Directory<I> {
    ...
    pub fn open(&self) -> Result<Vec<Item<I>>, OpenError> {
        ...
        loop {
            ...
            if ty.is_end() {
                break;
            } else if !ty.is_regular() {
                // Unused (deleted) entry: skip. Its former secondaries are
                // also unused and will be skipped by the same check.
                continue;
            } else if ty.type_category() != EntryType::PRIMARY {
                return Err(OpenError::NotPrimaryEntry(entry.index(), entry.cluster()));
            } else if ty.type_importance() != EntryType::CRITICAL {
                // Benign primary entry (e.g. Volume GUID): skip it and its
                // in-use secondaries (SecondaryCount is byte 1).
                for _ in 0..entry.data()[1] {
                    if let Err(e) = reader.read() {
                        return Err(OpenError::ReadEntryFailed(e));
                    }
                }
                continue;
            } else if ty.type_code() != 5 {
